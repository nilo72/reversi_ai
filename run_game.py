...
def main(**kwargs):
    ...
    if len(argv) <= 1 and len(kwargs) <= 1:
        ...
        print(
            '  size=(board size), amount=(#games), silent=(True/False), sim_time=(seconds for monte carlo sim)')
        quit()
    ...
    print('About to run {} games, black as {}, white as {}.'.format(
        amount, input_args['BlackAgent'].__name__, input_args[
            'WhiteAgent'].__name__))
    ...
    for t in range(1, amount + 1):
        ...
        message = '{} wins! {}-{}'.format(color_name[winner], white_score,
                                          black_score)
        info(message)
    ...
    print('time: {0:.2f} minutes ({0:.2f}ms per game)'.format(seconds_spent /
                                                              60, ms_per_game))
    print('summary: {} games played'.format(len(summary)))
    for each in summary:
        info(each)
    wins = {'Black': black_wins / (black_wins + white_wins) * 100,
